...
def test_migration_upgrade_creates_phase_1_tables() -> None:
    ...
    agent_config_indexes = {index["name"] for index in inspector.get_indexes("agent_config_versions")}
    admin_audit_indexes = {index["name"] for index in inspector.get_indexes("admin_audit_logs")}
    llm_call_indexes = {index["name"] for index in inspector.get_indexes("llm_call_logs")}
    tool_call_indexes = {index["name"] for index in inspector.get_indexes("tool_call_logs")}
    ...
    assert "ix_agent_config_versions_agent_name" in agent_config_indexes
    assert "ix_agent_config_versions_tenant_id" in agent_config_indexes
    assert "uq_agent_config_scope_agent_version" in agent_config_indexes
    assert "ix_admin_audit_logs_action" in admin_audit_indexes
    assert "ix_admin_audit_logs_tenant_id" in admin_audit_indexes
    assert "ix_llm_call_logs_run_id" in llm_call_indexes
    assert "ix_llm_call_logs_tenant_id" in llm_call_indexes
    assert "ix_tool_call_logs_run_id" in tool_call_indexes
    assert "ix_tool_call_logs_tool_name" in tool_call_indexes
