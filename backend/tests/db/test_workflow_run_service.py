...
@pytest.mark.skipif(not os.getenv("TEST_DATABASE_URL"), reason="requires TEST_DATABASE_URL")
@pytest.mark.asyncio
async def test_workflow_run_service_uses_assistant_summary_override_and_emits_progress_updates(
    db_session: AsyncSession,
) -> None:
    user_repository = UserRepository(db_session)
    tenant_repository = TenantRepository(db_session)
    ...
    user = await user_repository.create(external_auth_subject="subject-workflow-degraded-summary")
    tenant = await tenant_repository.create(name="Tenant Degraded", slug="tenant-degraded")
    thread = await thread_repository.create(
        tenant_id=tenant.id,
        created_by_user_id=user.id,
        active_workflow="account_search",
    ...
    await service.mark_running(
        tenant_id=tenant.id,
        run_id=run.id,
        status_detail="Started degraded account search.",
    )
    await service.emit_assistant_progress_update(
        tenant_id=tenant.id,
        run_id=run.id,
    ...
    assert any(
        message.message_type == "assistant_reply"
        and "i tried a backup source too" in message.content_text.lower()
        for message in run_messages
    )
