...
@pytest.mark.asyncio
async def test_account_research_service_creates_queued_run_with_optional_icp_context(
    ...
    service = AccountResearchService(db_session)

    user = await user_repository.create(
        external_auth_subject="subject-account-research-service",
    ...
    run = await service.create_account_research_run(
        ...
        research_objective="  evaluate buying readiness for acme's workflow offering  ",
        correlation_id=" corr-research-1 ",
    )

    assert run.workflow_type == "account_research"
    ...
    assert run.correlation_id == "corr-research-1"


@pytest.mark.skipif(not os.getenv("TEST_DATABASE_URL"), reason="requires TEST_DATABASE_URL")
@pytest.mark.asyncio
async def test_account_research_service_rejects_reviewer_access(
    db_session: AsyncSession,
) -> None:
    user_repository = UserRepository(db_session)
    tenant_repository = TenantRepository(db_session)
    ...
    reviewer = await user_repository.create(external_auth_subject="subject-account-research-reviewer")
    tenant = await tenant_repository.create(name="Tenant One", slug="tenant-one")
    ...
    with pytest.raises(ServiceError) as exc_info:
        await service.create_account_research_run(
            identity=AuthIdentity(
