...
from app.services.workflow_run_debug import WorkflowRunDebugService
...
@router.get(
    "/tenants/{tenant_id}/workflow-runs/{run_id}/debug",
    response_model=WorkflowRunDebugResponse,
)
async def get_workflow_run_debug(
    ...
    debug_bundle = await WorkflowRunDebugService(db_session).get_debug_bundle(
        identity=identity,
        tenant_id=tenant_id,
        run_id=run_id,
    )
    return WorkflowRunDebugResponse.model_validate(debug_bundle)
