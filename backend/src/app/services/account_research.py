...
ACCOUNT_RESEARCH_EDITOR_ROLES = {"owner", "admin", "member"}
...
class AccountResearchService:
    ...
    async def create_account_research_run(
        self,
        *,
        ...
        actor_user, _membership = await self._require_research_actor(
            identity=identity,
            tenant_id=tenant_id,
        )
        ...
        workflow_input = AccountResearchWorkflowInput(
            ...
            research_objective=_normalize_optional_text(research_objective),
        )
        return await self._run_service.create_queued_run(
            ...
            status_detail="Queued account research workflow run.",
        )

    async def _require_research_actor(
        self,
        *,
        identity: AuthIdentity,
        tenant_id: UUID,
        ...
        if membership.role not in ACCOUNT_RESEARCH_EDITOR_ROLES:
            raise ServiceError(
                status_code=403,
                error_code="tenant_membership_required",
                message="User does not have permission to run account research in this tenant.",
            )
