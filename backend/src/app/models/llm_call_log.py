...
LLM_CALL_LOG_STATUSES = ("completed", "failed")


class LlmCallLog(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "llm_call_logs"
    __table_args__ = (
        status_check("status", LLM_CALL_LOG_STATUSES, name="status_allowed"),
    )
    ...
    schema_hint: Mapped[str | None] = mapped_column(String(128), nullable=True)
    request_profile: Mapped[str | None] = mapped_column(String(128), nullable=True)
    ...
    estimated_cost_micros: Mapped[int | None] = mapped_column(nullable=True)
