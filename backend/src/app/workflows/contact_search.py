...
_CONTACT_SEARCH_FALLBACK_TRIGGER_ERROR_CODES = frozenset(
    {
        "provider_auth_error",
...
@dataclass(frozen=True)
class ContactSearchProviderExecution:
...
@dataclass(frozen=True)
class ContactSearchRecordExecution:
    record: ContactSearchRecord
    provider_failure_detected: bool = False
...
class ContactSearchWorkflow:
    ...
    async def execute(self, request: Any) -> WorkflowExecutionResult:
        try:
            ...
            provider_execution = await self._run_provider_search(
                tenant_id=request.tenant_id,
                run_id=request.run_id,
                account=account,
            ...
            record_execution = await self._build_contact_search_record(
                tenant_id=request.tenant_id,
                run_id=request.run_id,
                account=account,
                seller_profile=seller_profile,
                ...
                workflow_input=workflow_input,
                latest_snapshot=latest_snapshot,
                provider_response=provider_execution.response,
                search_results=search_results,
            )
            search_record = record_execution.record
            ...
            provider_failure_detected = (
                provider_execution.provider_failure_detected
                or record_execution.provider_failure_detected
            )
            if contact_ids:
                outcome = (
                    ContactSearchRunResultOutcome.CONTACTS_RANKED_VIA_FALLBACK
                    if provider_failure_detected and provider_execution.fallback_used
                    else ContactSearchRunResultOutcome.CONTACTS_RANKED
                )
            elif provider_failure_detected and provider_execution.fallback_attempted:
                outcome = ContactSearchRunResultOutcome.PROVIDER_FAILURE_WITH_FALLBACK_EXHAUSTED
            elif provider_failure_detected:
                outcome = ContactSearchRunResultOutcome.PROVIDER_FAILURE
            else:
            ...
            result = ContactSearchRunResult(
                ...
                primary_provider_name=provider_execution.primary_provider_name,
                fallback_provider_name=provider_execution.fallback_provider_name,
                primary_provider_failed=provider_execution.primary_provider_failed,
                fallback_attempted=provider_execution.fallback_attempted,
            ...
            return WorkflowExecutionResult(
                ...
                result_summary="Contact search hit an internal processing error before reliable results could be persisted.",
                error_code="contact_search_internal_error",
                status_detail=f"{type(exc).__name__}: {exc}",
                normalized_result_json={
                    "outcome": "contact_search_internal_error",
                    "assistant_summary": (
                    ...
                    "summary_selection_reason": (
                        "Selected internal contact-search failure summary because an unexpected "
                        "workflow exception interrupted candidate processing."
                    ),
                },
            )
    ...
    async def _run_provider_search(
        ...
        *,
    ...
    ) -> ContactSearchProviderExecution:
        ...
        if provider_search is None:
            return ContactSearchProviderExecution(
                response=None,
        ...
        await self._run_service.emit_provider_routing_decision(
            ...
            allowed=self._tools.fallback_provider_search is not None,
        ...
        primary_provider_failed = response.error_code in _CONTACT_SEARCH_FALLBACK_TRIGGER_ERROR_CODES
        ...
        if not should_fallback:
            return ContactSearchProviderExecution(
                ...
                fallback_provider_name=fallback_provider_name,
                provider_failure_detected=primary_provider_failed,
        ...
        await self._run_service.emit_provider_routing_decision(
            ...
            trigger_reason=response.error_code or "explicit_no_results",
            allowed=True,
            reason_summary=(
        ...
        await self._run_service.emit_assistant_progress_update(
            ...
            run_id=run_id,
            content_text=(
                "I couldn't confirm enough reliable contacts from the primary source, "
                "so I'm trying a backup source now."
            ),
        )
        await self._run_service.emit_tool_started(
            tenant_id=tenant_id,
        ...
        return ContactSearchProviderExecution(
            ...
            fallback_used=bool(fallback_response.candidates),
        )
    ...
    async def _build_contact_search_record(
        ...
        *,
        ...
        await self._run_service.emit_tool_completed(
            ...
            tool_name="content_normalizer",
            provider_name=content_normalizer_provider,
            output_summary=response.raw_result_summary or (
                f"Ranked {len(merged_record.contacts)} normalized contact candidate(s)."
            ),
            error_code=response.error_code,
            produced_evidence_results=bool(merged_record.contacts),
        )
        return ContactSearchRecordExecution(
            record=merged_record,
            provider_failure_detected=provider_failure_without_payload,
        )
...
def _select_terminal_assistant_summary(
    *,
    ...
    if outcome is ContactSearchRunResultOutcome.CONTACTS_RANKED_VIA_FALLBACK:
        return (
            "I had trouble with one of our main contact sources, but I was able to continue with a backup search and found a smaller set of contact candidates.",
            "Selected degraded-success summary because the primary contact provider failed and fallback produced ranked contacts.",
        )
    if outcome is ContactSearchRunResultOutcome.PROVIDER_FAILURE_WITH_FALLBACK_EXHAUSTED:
        return (
            "Hmm, looks like one of our contact sources is down. I tried a backup source too, but I couldn't confirm any reliable contacts from the available data.",
            "Selected degraded-failure summary because the primary contact provider failed, fallback ran, and no reliable contacts were confirmed.",
        )
    if outcome is ContactSearchRunResultOutcome.PROVIDER_FAILURE:
        return (
            "Hmm, looks like one of our contact sources is down, and I couldn't continue with a reliable contact-search path from the available data.",
            "Selected provider-failure summary because an upstream contact provider failure prevented a trustworthy ordinary no-results outcome.",
        )
    if contact_count == 0:
        return (
            f"I finished the contact search workflow. {reason_summary.strip()}",
            "Selected true no-results summary because contact search exhausted the available evidence without a known upstream provider outage.",
        )
    return (
        f"I finished the contact search workflow. {reason_summary.strip()}",
        "Selected normal success summary because ranked contact candidates were produced on the non-degraded path.",
    )
