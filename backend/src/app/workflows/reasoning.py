...
class AccountSearchQueryPlanOutput(ReasoningModel):
...
def build_account_search_query_plan_prompt_spec() -> str:
    return (
        "Plan account-search queries for nuanced B2B prospect discovery. "
        "Generate concise, web-search-ready query ideas grounded in seller context, ICP fit, "
        "and prior attempt outcomes. Prefer semantic targeting over generic restatements."
    )
...
def validate_account_search_query_plan(
    payload: dict[str, Any] | list[dict[str, Any]] | None,
) -> AccountSearchQueryPlanOutput | None:
    if not isinstance(payload, dict):
        return None
    normalized_payload = dict(payload)
    if "queries" in normalized_payload and "query_ideas" not in normalized_payload:
        normalized_payload["query_ideas"] = normalized_payload.pop("queries")
    if "strategy" in normalized_payload and "search_strategy" not in normalized_payload:
        normalized_payload["search_strategy"] = normalized_payload.pop("strategy")
    if "query_ideas" not in normalized_payload:
        normalized_payload["query_ideas"] = []
    if "fit_criteria" not in normalized_payload:
        normalized_payload["fit_criteria"] = []
    if "clarification_questions" not in normalized_payload:
        normalized_payload["clarification_questions"] = []
    return _validate_model(AccountSearchQueryPlanOutput, normalized_payload)
...
def is_provider_failure_without_payload(
    *,
    error_code: str | None,
    payload: dict[str, Any] | list[dict[str, Any]] | None,
) -> bool:
